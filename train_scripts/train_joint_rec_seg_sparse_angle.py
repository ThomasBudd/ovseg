...
import os
from ovseg.training.JoinedTraining import JoinedTrainingV2
...
model_name_recon = 'recon_fbp_convs_2000_eights_8_32'

proj_folder = 'projections_eights'
im_folder = model_name_recon

val_fold = 0
data_name = 'OV04'
preprocessed_name = 'pod_no_resizing'
model_name_seg = 'segmentation_pod_no_resizing_pretrained'
...
for loss_weight in loss_weights:
    for i in range(3):
        # %%
        model_path = os.path.join(os.environ['OV_DATA_BASE'], 'trained_models',
                                  data_name, 'joined_sparse_{:.4f}_{}'.format(loss_weight, i))
        if args.fp32:
            model_path += '_fp32'
        training = JoinedTrainingV2(model1, model2, data.trn_dl,  model_path,
        ...
        recon_path = os.path.join(os.environ['OV_DATA_BASE'], 'predictions', args.data,
                                  'joined_sparse_{:.4f}_{}'.format(loss_weight, i),
                                  'reconstructions')
