...
def torch_interp_img(img, grid, order, cval=None):
    ...
    if order == 0:
        ...
        if idim == 2:
            img_trsf = img[inds[0], inds[1]]
        elif idim == 3:
            img_trsf = img[inds[0], inds[1], inds[2]]
        else:
            raise ValueError('grid must be of shape (2,nx,ny) for 2d or '
                             + '(3,nx,ny,nz) for 3d interpolation.'
                             ' Got {}'.format(grid.shape))
        ...
        if idim == 2:
            ...
            # do the transformation all at once, this saves memory and is not an in place
            # operation
            img_trsf = (1 - xi[0]) * (1 - xi[1]) * img[inds[0], inds[1]]\
                + xi[0] * (1 - xi[1]) * img[inds[0] + 1, inds[1]]\
                + (1 - xi[0]) * xi[1] * img[inds[0], inds[1] + 1]\
                + xi[0] * xi[1] * img[inds[0] + 1, inds[1] + 1]

        elif idim == 3:
            # do the transformation all at once, this saves memory and is not an in place
            # operation
            img_trsf = (1 - xi[0]) * (1 - xi[1]) * (1 - xi[2]) * img[inds[0], inds[1], inds[2]]\
                + xi[0] * (1 - xi[1]) * (1 - xi[2]) * img[inds[0] + 1, inds[1], inds[2]]\
                + (1 - xi[0]) * xi[1] * (1 - xi[2]) * img[inds[0], inds[1] + 1, inds[2]]\
                + xi[0] * xi[1] * (1 - xi[2]) * img[inds[0] + 1, inds[1] + 1, inds[2]]\
                + (1 - xi[0]) * (1 - xi[1]) * xi[2] * img[inds[0], inds[1], inds[2] + 1]\
                + xi[0] * (1 - xi[1]) * xi[2] * img[inds[0] + 1, inds[1], inds[2] + 1]\
                + (1 - xi[0]) * xi[1] * xi[2] * img[inds[0], inds[1] + 1, inds[2] + 1]\
                + xi[0] * xi[1] * xi[2] * img[inds[0] + 1, inds[1] + 1, inds[2] + 1]
        else:
            raise ValueError('grid must be of shape (2,nx,ny) for 2d or '
                             + '(3,nx,ny,nz) for 3d interpolation.'
                             ' Got {}'.format(grid.shape))
