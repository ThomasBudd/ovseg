...
from ovseg.data.RegionfindingDataloader import RegionfindingDataloader
from os import listdir
from os.path import join


class RegionfindingData(DataBase):
    ...
    def initialise_dataloader(self, is_train):
        if is_train:
            ...
            self.trn_dl = RegionfindingDataloader(self.trn_ds,
                                                 augmentation=self.augmentation,
                                                 **self.trn_dl_params)
        else:
            print('Initialise validation dataloader')
            try:
                self.val_dl = RegionfindingDataloader(self.val_ds,
                                                     augmentation=self.augmentation,
                                                     **self.val_dl_params)
