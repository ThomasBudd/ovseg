...
parser.add_argument("-lw", "--loss_weights", nargs='+', required=False, default='0.75')
...
if parser.data.lower() == 'ov':
    ...
    seg_prep_mn = 'segmentation_pretrain'
...
for loss_weight in loss_weights:
    ...
    trn_dl_params = {'batch_size': 12, 'patch_size': [512, 512],
                     ...
                     'epoch_len': 25, 'store_coords_in_ram': True}
    val_dl_params = {'batch_size': 12, 'patch_size': [512, 512],
                     'num_workers': None, 'pin_memory': True,
                     'epoch_len': 2, 'store_coords_in_ram': True, 'store_data_in_ram': True,
                     'n_max_volumes': 20}
    ...
    # %% load models
    print('create reconstruction model:')
    sleep(3)
    ...
    sleep(3)
    model2 = SegmentationModel(val_fold, data_name, seg_prep_mn,
    ...
    # %%
    model_path = os.path.join(os.environ['OV_DATA_BASE'], 'trained_models',
                              data_name, 'test_joined_{}_{}'.format(loss_weight, simulation))
