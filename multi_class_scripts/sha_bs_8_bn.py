...
parameter_names = [['training', 'opt_params', 'weight_decay'],
                   ['training', 'lr_params', 'lr_max']]

# %%
parameter_grids = [np.logspace(np.log10(1e-6), np.log10(1e-3), 10),
                   np.logspace(np.log10(1e-3), np.log10(1e-1), 8)]

print('wd')
print(', '.join(['{:.3e}'.format(p) for p in parameter_grids[0]]))
print('lr')
print(', '.join(['{:.3e}'.format(p) for p in parameter_grids[1]]))
# %%
...
default_model_params['data']['trn_dl_params']['min_biased_samples'] = 4
