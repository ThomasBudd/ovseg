...
cross[1, :] = 1/5
cross[:, 1] = 1/5
im_gpu = torch.from_numpy(im).cuda()
im_oh = torch.stack([im_gpu==1, im_gpu==2], 0).unsqueeze(0).type(torch.float)
circ_gpu = torch.stack(2*[torch.from_numpy(circ).cuda().unsqueeze(0)]).type(torch.float)
cross_gpu = torch.stack(2*[torch.from_numpy(cross).cuda().unsqueeze(0)]).type(torch.float)

im_oh_conv = torch.nn.functional.conv2d(im_oh, circ_gpu, padding=(r,r), groups=2)
im_oh_dial = (im_oh_conv[0] > 0).type(torch.float).cpu().numpy()

im_oh_conv = torch.nn.functional.conv2d(im_oh, cross_gpu, padding=(1,1), groups=2)
im_oh_eros_edge = (im_oh - (im_oh_conv[0] >= 1).type(torch.float)).cpu().numpy()

im_am = np.argmax(np.concatenate([np.zeros((1, 128, 128)), im_oh_dial], 0), 0)

plt.subplot(2, 4, 2)
plt.imshow(im_oh_eros_edge[0, 0])
plt.subplot(2, 4, 3)
plt.imshow(im_oh_eros_edge[0, 1])
...
ovlp = (np.sum(im_oh_dial, 0) > 1).astype(float)
plt.subplot(2, 4, 6)
plt.imshow(ovlp)
...
X, Y = np.where(ovlp > 0)
X1, Y1 = np.where(im_oh_eros_edge[0, 0])
X2, Y2 = np.where(im_oh_eros_edge[0, 1])

for x, y in zip(X, Y):

    min_dist1 = np.min([(x-x1)**2+(y-y1)**2 for x1, y1 in zip(X1, Y1)])
    min_dist2 = np.min([(x-x2)**2+(y-y2)**2 for x2, y2 in zip(X2, Y2)])
...
# %%
im3d = np.stack([im, im2, im3, im3])
im3d_dial = seg_fg_dial_2d_stacked(im3d, r)

plt.figure()
for i in range(3):
    plt.subplot(2, 3, 1+i)
    plt.imshow(im3d[i], vmax=3)
for i in range(3):
    plt.subplot(2, 3, 4+i)
    plt.imshow(im3d_dial[i], vmax=3)

# %%
# %%
# def seg_fg_dial_3d(seg, r, z_to_xy_ratio=1):
# performs the segmentation foreground dilation for each slice. Expects the z axis
# to be in first dimension
...
# %%
im3d = np.stack([im,im, im2, im3, im3])
im3d_dial = seg_fg_dial_3d(im3d, r, 7)
